...
import { API_BASE_URL, ORGANIZATION_ID, getJson } from "../../lib/launch-os";
...
export default function RecommendationPage() {
  ...
  async function loadPreview() {
    ...
    try {
      const response = await getJson<{ data: RecommendationPreviewData }>(`/recommendation/preview?organizationId=${ORGANIZATION_ID}`);
  ...
  return (
    <WorkflowShell
      ...
      description={`Preview generated from the saved workflow draft via ${API_BASE_URL}/recommendation/preview.`}
    >
      {isLoading ? <p>Loading recommendation...</p> : null}
      {error ? (
        <div>
          <p role="alert">{error}</p>
          <button type="button" onClick={() => void loadPreview()}>Retry</button>
        </div>
      ) : null}
      {!isLoading && !error && data ? (
        <div style={{ display: "grid", gap: 16 }}>
          ...
          <section>
            <h2>Reasons</h2>
            <ul>
              {data.result.explainability.reasons.map((reason) => (
          ...
          <section>
            ...
            <p><strong>Suggested vendors:</strong> {data.result.stackFitSummary.data.suggestedVendorIds.join(", ") || "None yet"}</p>
            <ul>
              {data.result.stackFitSummary.data.fitNotes.map((note) => (
                <li key={note}>{note}</li>
              ))}
            </ul>
            <p><strong>Suggested baselines:</strong> {data.result.securityBaselineSummary.data.suggestedBaselineCodes.join(", ") || "None yet"}</p>
            <ul>
              {data.result.securityBaselineSummary.data.rationale.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </section>
...
function PolicySection({ title, breakdown }: { title: string; breakdown: PolicyBreakdown }) {
  return (
    <article>
      ...
      <ul>
        {breakdown.negativeSignals.map((signal) => (
          <li key={signal}>{signal}</li>
        ))}
      </ul>
    </article>
