...
export default function BusinessModelPage() {
  ...
  useEffect(() => {
    setDraft(readStoredDraft(STORAGE_KEYS.businessModel, defaultDraft));
  }, []);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await postJson<{ data: { id: string; targetVerticals: string[]; targetCompanySizes: string[] } }>("/business-model", payload);
      writeStoredDraft(STORAGE_KEYS.businessModel, {
        ...draft,
        id: response.data.id,
        targetVerticalsText: response.data.targetVerticals.join(", "),
        targetCompanySizesText: response.data.targetCompanySizes.join(", ")
      });
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <label>
          Business type
          <select value={draft.businessType} onChange={(event) => update("businessType", event.target.value as BusinessModelDraft["businessType"])}>
        ...
        <label>
          ...
          <input value={draft.targetVerticalsText} onChange={(event) => update("targetVerticalsText", event.target.value)} required />
        </label>
        <label>
          Target company sizes (comma separated)
          <input value={draft.targetCompanySizesText} onChange={(event) => update("targetCompanySizesText", event.target.value)} required />
        </label>
        <label>
          Delivery model
          <select value={draft.deliveryModel} onChange={(event) => update("deliveryModel", event.target.value as BusinessModelDraft["deliveryModel"])}>
        ...
        <label>
          ...
          <select value={draft.complianceSensitivity} onChange={(event) => update("complianceSensitivity", event.target.value as BusinessModelDraft["complianceSensitivity"])}>
        ...
        <label>
          Budget positioning
          <select value={draft.budgetPositioning} onChange={(event) => update("budgetPositioning", event.target.value as BusinessModelDraft["budgetPositioning"])}>
        ...
        <label>
          Founder maturity
          <select value={draft.founderMaturity} onChange={(event) => update("founderMaturity", event.target.value as BusinessModelDraft["founderMaturity"])}>
        ...
        <label>
          Revenue strategy
          <select value={draft.revenueStrategy} onChange={(event) => update("revenueStrategy", event.target.value as BusinessModelDraft["revenueStrategy"])}>
        ...
        <label>
          ...
          <input
            ...
            min={0}
            max={100}
            value={draft.targetGrossMarginPercent}
            onChange={(event) => update("targetGrossMarginPercent", Number(event.target.value))}
            required
          />
        </label>
